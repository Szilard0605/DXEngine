...
Mesh::Mesh(std::vector<StaticMeshVertex> vertices, std::vector<uint32_t> indices, PBRMaterial& material)
	: m_Vertices(vertices), m_Indices(indices), m_Material(material)
{
	...
	m_VertexBuffer->SetData(m_Vertices.data(), static_cast<uint32_t>(m_Vertices.size() * sizeof(StaticMeshVertex)));
...
Mesh Mesh::ImportStaticMesh(std::filesystem::path path)
{
